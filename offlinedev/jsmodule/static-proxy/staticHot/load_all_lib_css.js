...
module.exports = {
    ...
    shouldReplacedWithThis:(sourceDir, req_realpath)=>{
        if(req_realpath.match(/\.css$/)){
            let bi_path = pathutil.resolve(sourceDir, './lib')
            if(is_path_inside(req_realpath, bi_path)){
                return 'platform/core/css/all-lib_HOT.css';
            }
        }
    },
    load:(webappFolder, callback)=>{
        let destFile = pathutil.resolve(webappFolder, './static/source/platform/core/css/all-lib_HOT.css')
        updateCss.concatCss({
            sourceDirList: [
                pathutil.resolve(webappFolder, './static/source/lib')
            ],
            filterFun:()=>{
                return true;
            },
