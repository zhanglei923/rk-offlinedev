...
let init = (all_trans, all_untrans)=>{
    ...
    for(var i = 0, len=allKeys.length;i<len;i++){
        ...
        html = html + `<tr class="row ${count%2===0?'row_a':'row_b'}" data-key="${key}" title="" style="${hidden?'display:none;':''}">`+
                            `<td>#${count+1}</td>`+
                            `<td class="cellkey" title="${belongpath}">${key}<div class="untrans_path">${belongpath}</div></td>`+
                            `<td class="cellval">`+
                            `<div class="cellmainlang word">${getDisplayText(cnvalue, 'cn')}<div class="cellfilepath word">${belongpath}</div></div>`+
